function integersList = extractIntegersFromFilenames(patient_id, comp_options, enc_id, image_id, params)
    file_to_load = sprintf('%s\\%s\\session%d\\%s\\%s\\enc%s_image%s\\', ...
        params.output_folder, num2str(patient_id), params.session_id, comp_options{1}, params.btwn_trial_type, num2str(enc_id), num2str(image_id));
