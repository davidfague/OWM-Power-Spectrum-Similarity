function [Zpower] = compute_Zpower(data)
    % 1 channel selected; 1 trial selected
    % data should be 1x9001 1xnTimes
    npts = length(data);
    
    % Define parameters for time-frequency analysis
    Fre = 1:40; % Frequency range from 1 to 100 Hz
    Fsample = 1000; % Sampling frequency in Hz
    wavenum = 6; % Number of cycles in the wavelet
    power = zeros(1, 1, length(Fre), npts);
    
    [B, T, F] = BOSC_tf_power(data, Fre, Fsample, wavenum); % Compute time-frequency power
    power(1, 1, :, :) = B; % Store the computed power
    ...
    base.powspctrm = power(1,1,:,bt); % Extract the baseline power spectrum
