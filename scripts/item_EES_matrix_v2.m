...
%% main code
same_n = true; % Enable same_n feature
...
for chan_id = channel_ids_to_use
    ...
    % Initialize a variable to track the minimum trial pair count
    min_trial_pairs = inf;

    % First loop: Calculate minimum trial pair count and store Ns
    for first_image_id = image_ids
        for second_image_id = image_ids
            ...
            % Compute trial pair count
            trial_pair_count = size(data_btwn_these_two_images, 3) * size(data_btwn_these_two_images, 4);
            if trial_pair_count > 0
                min_trial_pairs = min(min_trial_pairs, trial_pair_count);
            end

            % Store the trial pair count
            Ns(first_image_id, second_image_id, :) = trial_pair_count;
        end
    end

    % Adjust min_trial_pairs to ensure divisibility by the 4th dimension
    fourth_dim_size = size(data_btwn_these_two_images, 4); % Store 4th dimension size
    if mod(min_trial_pairs, fourth_dim_size) ~= 0
        % Reduce to the largest value divisible by the 4th dimension
        min_trial_pairs = floor(min_trial_pairs / fourth_dim_size) * fourth_dim_size;
    end

    % Second loop: Process data with `same_n` constraint
    for first_image_id = image_ids
        for second_image_id = image_ids
            % Get data
            [data_btwn_these_two_images, ~] = get_data_btwn_these_two_images(first_image_id, second_image_id, patient_id, comp_options, enc_id, chan_id, params);

            % Apply `same_n` constraint if enabled
            trial_pair_count = size(data_btwn_these_two_images, 3) * size(data_btwn_these_two_images, 4);
            if same_n && trial_pair_count > min_trial_pairs
                % Flatten dimensions 3 and 4 into a single dimension
                total_elements = trial_pair_count;
                reshaped_data = reshape(data_btwn_these_two_images, [], total_elements);

                % Randomly subsample to match the adjusted minimum trial pair count
                rng(42);
                subsampled_indices = randperm(total_elements, min_trial_pairs);
                subsampled_data = reshaped_data(:, subsampled_indices);

                % Reshape back to match original dimensions
                new_dim3 = min_trial_pairs / fourth_dim_size;

                % Verify `new_dim3` is a valid integer
                if mod(min_trial_pairs, fourth_dim_size) ~= 0 || ~isequal(new_dim3, round(new_dim3))
                    error('Adjusted min_trial_pairs resulted in invalid reshape dimensions. Debug Info: min_trial_pairs=%d, fourth_dim_size=%d', min_trial_pairs, fourth_dim_size);
                end

                % Perform the reshape
                data_btwn_these_two_images = reshape(subsampled_data, size(data_btwn_these_two_images, 1), size(data_btwn_these_two_images, 2), round(new_dim3), fourth_dim_size);
            end
