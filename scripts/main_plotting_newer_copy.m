...
for row_idx = 1:length(all_patients_p_table.patient_id)
    plot_params.chan_id = all_patients_p_table(row_idx,:).chan_id;
    plot_params.image_id = all_patients_p_table(row_idx,:).image_id;
    plot_params.patient_id = all_patients_p_table(row_idx,:).patient_id;
    params.k12wm = plot_params.patient_id < 1000;
    plot_params.anat = all_patients_p_table(row_idx,:).anat;
    plot_params.anat_merged = all_patients_p_table(row_idx,:).anat_merged;
    
    plot_params.p = all_patients_p_table(row_idx,:).p;
    ...
    % if mean(ismember(char("temp"), char(lower(plot_params.anat)))) ==1
    if ismember(plot_params.anat_merged, ["middle temporal gyrus", "middle frontal gyrus"])
        returned_values = plot_WI_vs_BI(params, plot_params);
    end
    close all
end
...
function to_return = plot_WI_vs_BI(params, plot_params)
    tic
    patient_id = [plot_params.patient_id];
    ...
    for session_idx = 1:length(patient_preprocessed_data_paths)
        ...
        plot_params.WI_BI_folder_to_save_in = sprintf("results/WI vs BI/p%s chan%s image%s enc%s sess%d %s %0.2f", ...
                    num2str(plot_params.patient_id), num2str(plot_params.chan_id), ...
                    num2str(plot_params.image_id), num2str(plot_params.enc_id), session_idx, plot_params.anat, plot_params.p);
        ...
        for chan_idx = 1:length(channel_ids_to_use)
            ...
            if plot_params.test_significance
                ...
                if ~exist(sprintf("%s/significant_cluster_data.mat", plot_params.WI_BI_folder_to_save_in), 'file')
                    ...
                    fprintf("    calculating real t-tests\n")
                    [real_t_values, real_p_values] = calc_ttest(diff);
                    to_return.real_t_values=real_t_values;
                    to_return.real_p_values=real_p_values;
                
                    % calculate 1000 surrogate differences and t-tests
                    fprintf("    calculating surrogate differences and t-tests\n")
                    [surrogate_t_values, surrogate_p_values, surrogate_diff] = calc_surrogate(WI, BI, 1000, plot_params.average_diff, plot_params.mean_out_time_dimensions);
                    save(sprintf("%s/significant_cluster_data.mat", plot_params.WI_BI_folder_to_save_in),"real_p_values", "real_t_values", "surrogate_t_values", "surrogate_p_values", "surrogate_diff")
                    to_return.surrogate_t_values=surrogate_t_values;
                    to_return.surrogate_p_values =surrogate_p_values;
                    to_return.final_p=final_p;
                else
