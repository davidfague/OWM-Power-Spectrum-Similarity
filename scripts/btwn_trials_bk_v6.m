...
%v6 enable switching between "EES" and "EMS"; improved heatmaps subtitles;
%improved plot labels all around
...
% channels_to_use = (contains(string(label_table{ia, 3}), 'inf', 'IgnoreCase', true) & ...
%                    contains(string(label_table{ia, 3}), 'temp', 'IgnoreCase', true)) | ...
%                    contains(string(label_table{ia, 3}), 'ifg', 'IgnoreCase', true);
...
for chan_id = channel_ids_to_use
    ...
    if any(isnan(within_trial_single_trial(:)))
        warning("nan in this within trial data. change test_trial_to_use? skipping channel %s for now.", num2str(chan_id))
        continue
    end
    ...
    for control_trial_to_use = control_trials_to_use
        ...
        % Subplot 2(col1 after row1): Between-trial Within-Image single-trial pair
        ...
        title(strcat(comp_titles{2}, sprintf(' %s vs %s', num2str(test_trial_to_use), num2str(control_trial_to_use))))
        add_single_trial_WT_vs_BTWI_vs_BTBI_labels()
    
        % Subplot 3(col2 after row1): Between-trial Between-Image single-trial pair 
        ...
        title(strcat(comp_titles{1}, sprintf(' %s vs %s', num2str(test_trial_to_use), num2str(control_trial_to_use))))% n: % , num2str(size(BT_BI_data.BT_ES(:,:,control_trial_to_use,test_trial_to_use),3))))
        add_single_trial_WT_vs_BTWI_vs_BTBI_labels()
    ...
    if strcmp(params.btwn_trial_type, 'EES')
        subplot(1+length(control_trials_to_use), 2, iter) % end col1 (WI)
        xlabel(sprintf("enc%s windows", num2str(enc_id)))
        subplot(1+length(control_trials_to_use), 2, 1+iter) % end col2 (BI)
        xlabel("enc1-3 windows")
    elseif strcmp(params.btwn_trial_type, 'EMS')
        subplot(1+length(control_trials_to_use), 2, iter) % end col1 (WI)
        xlabel("maint windows")
        subplot(1+length(control_trials_to_use), 2, 1+iter) % end col2 (BI)
        xlabel("maint windows")
    end


    save_file = fullfile(params.output_destination, sprintf('single_trial_WT_vs_BTWI_vs_BTBI_heatmaps/%s/Patient%d_Channel%d_Enc%d_Image%d.fig', ...
    params.btwn_trial_type, patient_id, chan_id, enc_id, image_id));
...
for chan_id = channel_ids_to_use
    ...
    if same_n
        save_file = fullfile(params.output_destination, sprintf('single_trial_WT_vs_BTWI_vs_BTBI_lines_same_n/%s/Patient%s_Channel%s_Enc%s_Image%s.fig', ...
        params.btwn_trial_type, num2str(patient_id), num2str(chan_id), num2str(enc_id), num2str(image_id)));
    else
        save_file = fullfile(params.output_destination, sprintf('single_trial_WT_vs_BTWI_vs_BTBI_lines/%s/Patient%d_Channel%d_Enc%d_Image%d.fig', ...
        params.btwn_trial_type, patient_id, chan_id, enc_id, image_id));
    end
...
function [all_channels_data] = load_all_channels_data(params, patient_id, comp_options, enc_id, image_id)
    all_channels_data = load(sprintf('%s\\%s\\%s\\%s\\enc%d_image%d\\all_channels_data.mat', ...
        params.output_folder, num2str(patient_id), comp_options{2}, params.btwn_trial_type, enc_id, image_id));
end
...
for chan_id = channel_ids_to_use
    ...
    % remove infs from within trial EES.
    flattenedMatrix = reshape(BT_WI_data.BT_ES, [], size(BT_WI_data.BT_ES, 3), size(BT_WI_data.BT_ES, 4));
    flattenedMatrix = squeeze(any(isinf(flattenedMatrix), 1));
    % figure;
    % imagesc(flattenedMatrix)
    BT_WI_data.BT_ES = BT_WI_data.BT_ES(:, :, test_trials_without_nan, test_trials_without_nan);
    ...
    for test_trial_to_use = test_trials_to_use
        ...
        control_trials_to_use = 1:size(BT_WI_data.BT_ES, 3); % WI
        control_trials_to_use(control_trials_to_use == test_trial_to_use) = []; % Removes all occurrences of 'test_trial within control_trials_to_use' to avoid inf in EES, and EMS-biased-by-within-trial.
        control_trials_to_use_BT_BI = 1:size(BT_BI_data.BT_ES, 3);
...
output_table_path = fullfile(params.output_destination, "WIvsBI Table Summary", params.btwn_trial_type);
if ~exist(output_table_path, "dir")
    mkdir(output_table_path)
end
save(fullfile(output_table_path, "WIvsBI_summary_table.mat"), "summary_table", "groupedby_anat_summary");

%% view average as a function of trial_idx to see if improved replay over time.
y_limits = [0 1];
replace_inf_with_one = true;
if strcmp(params.btwn_trial_type, 'EMS')
    window2_type = 'M';
elseif strcmp(params.btwn_trial_type, 'EES')
    window2_type = 'E2';
...
for chan_id = channel_ids_to_use
    ...
    % set inf to 1
    if replace_inf_with_one
        BT_WI_data.BT_ES(isinf(BT_WI_data.BT_ES)) = 1;
    end
    ...
    ylim(y_limits)
    xlabel(sprintf('%s trial_idx', window2_type))
    subplot(4,1,2)
    for trial_idx = 1:num_trials
    ...
    xlabel(sprintf('E trial_idx - %s trial_idx', window2_type));
    ylim(y_limits)
    subplot(4,1,4)
    for trial_idx = 1:num_trials
    ...
    ylim(y_limits)
    xlabel(sprintf('E trial_id - %s trial_id', window2_type))
    subplot(4,1,3)
    for trial_idx = 1:num_trials
    ...
    ylabel('mean EMS');
    xlabel(sprintf('%s trial_id', window2_type))
    legend(legend_entries, 'Location', 'best');
    ylim(y_limits)
    hold off;

    if replace_inf_with_one
        sgtitle([sprintf('%s Mean Patient: %d, Channel: %d, Enc: %d, Image: %s *REPLACED INF WITH 1', ...
    params.btwn_trial_type, patient_id, chan_id, enc_id, C{1, image_id}), ...
    unique(string(BT_WI_data.chan_test_table.anatomical_label))]);
    else
        sgtitle([sprintf('%s Mean Patient: %d, Channel: %d, Enc: %d, Image: %s', ...
    params.btwn_trial_type, patient_id, chan_id, enc_id, C{1, image_id}), ...
    unique(string(BT_WI_data.chan_test_table.anatomical_label))]);
    end

    save_file = fullfile(params.output_destination, sprintf('BTWI_thru_trial_idx/%s/Patient%d_Channel%d_Enc%d_Image%d.fig', ...
    params.btwn_trial_type, patient_id, chan_id, enc_id, image_id));
    save_dir = fileparts(save_file);
    if ~exist(save_dir, 'dir')
        mkdir(save_dir);
    ...
    % 
    % 
    % sgtitle([sprintf('Min-Max-Normalized %s Patient: %d, Channel: %d, Enc: %d, Image: %s', ...
    % params.btwn_trial_type, patient_id, chan_id, enc_id, C{1, image_id}), ...
    % unique(string(BT_WI_data.chan_test_table.anatomical_label))]);
    % 
    % save_file = fullfile(params.output_destination, sprintf('BTWI_thru_trial_idx_MinMaxNormalized/%s/Patient%d_Channel%d_Enc%d_Image%d.fig', ...
    % params.btwn_trial_type, patient_id, chan_id, enc_id, image_id));
    % save_dir = fileparts(save_file);
    % if ~exist(save_dir, 'dir')
    %     mkdir(save_dir);
