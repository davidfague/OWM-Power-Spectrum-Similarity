...
patient_IDs = [201907 201908, 201910, 201915, 201903, 201905, 201906, 201901];
...
%     parpool('local', 2); % Open a local parallel pool if none exists
% end
...
for idx = 1:length(patient_IDs)
    ...
    raw_data_file = matfile(fullfile('Raw Data Storage', ['D_OWM_t_bipolar_', num2str(patient_ID), '.mat']));
    ...
    gamma_data = matfile(fullfile('Raw Data Storage', ['gammachans2sd_alltrials_', num2str(patient_ID), '.mat']));
    is_gamma_channels = bool_mask_array(gamma_data.sigchans2, num_channels); % converts ([1,3,5], 8) to [1, 0, 1, 0, 1, 0, 0, 0]

    % clear variables no longer needed

    %% generate large data subset slice indices
    ...
    %% I think I will process each trial and then subset in post-processing to avoid reprocessing entire trials.
    ...
    rows_with_nan = any(isnan(label_table.encoding_correctness), 2);
    ...
    all_windowed_mean_PS_vectors = preallocate_all_windowed_mean_PS_vectors(...
        raw_data_file.D_OWM_t(channels_to_process(1), :, trials_to_process(1)), ...
        num_iterations);
