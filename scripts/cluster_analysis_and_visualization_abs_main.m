function [final_p_this_obs, all_real_clusters, significant_clusters, sum_threshold, alpha] = cluster_analysis_and_visualization_abs_main(real_t_values, real_p_values, surrogate_t_values, surrogate_p_values, plot_params)
    % Parameters
    ...
    colormap('jet')
    colorbar;
    saveas(fig, sprintf("%s/WI_BI_raw_clusters.png", plot_params.WI_BI_folder_to_save_in))
    ...
    clim([-30, 30])
    title('Null')
    saveas(fig, sprintf("%s/WI_BI_raw_t_values.png", plot_params.WI_BI_folder_to_save_in))
    ...
    for i = 1:num_surrogates
        ...
        if surrogate_positive_clusters.NumObjects == 0
        ...
        if surrogate_negative_clusters.NumObjects == 0
    ...
    fig = plot_cluster_sum_distribution(all_surrogate_sums, all_real_cluster_sums, significance_threshold, final_p_this_obs);
    saveas(fig, sprintf("%s/WI_BI_Cluster Tsum distribution.png", plot_params.WI_BI_folder_to_save_in))
    %%
    ...
    fig = visualize_clusters(real_p_values, real_t_values, all_real_clusters, significant_clusters, sum_threshold, alpha, final_p_this_obs);
    saveas(fig, sprintf("%s/WI_BI_cluster_visualizastion.png", plot_params.WI_BI_folder_to_save_in))
end
