...
for idx = 1:length(patient_IDs)
    ...
    compute_all_between_trial_similarities(PS_file)
...
function [] = compute_all_between_trial_similarities(data_file)
    ...
    %% Set up save folder
    save_folder = strrep(data_file.Properties.Source, '.mat', '_all3corrWOitem1_and_item1correctW');
    if ~exist(save_folder, 'dir')
        mkdir(save_folder);
    end
    ...
    for chan_idx = 1:length(unique_channel_IDs)
        ...
        save(save_file, "chan_id", "chan_test_table", "chan_ctrl_table", "test_EMS_matrices", "control_EMS_matrices");
    end

    % Save all channel data in a final file
    all_channels_save_file = fullfile(save_folder, 'all_channels_data.mat');
    save(all_channels_save_file, "item_cor_table", "control_table", "all_test_EMS_matrices_by_chan", "all_control_EMS_matrices_by_chan", "unique_channel_IDs");
end
