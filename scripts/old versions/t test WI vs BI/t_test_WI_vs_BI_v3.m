...
function [BT_BI_data, BT_WI_data] = load_BT_data(patient_id, comp_options, enc_id, image_id, chan_id, params, btwn_trial_type)
    ...
    BT_BI_data = load(sprintf('%s\\%s\\%s\\%s\\enc%s_image%s\\BT_%s.mat', ...
        params.output_folder, num2str(patient_id), comp_options{1}, btwn_trial_type, num2str(enc_id), num2str(image_id), num2str(chan_id)));
    clear BT_ES
    ...
    BT_WI_data = load(sprintf('%s\\%s\\%s\\%s\\enc%s_image%s\\BT_%s.mat', ...
        params.output_folder, num2str(patient_id), comp_options{2}, btwn_trial_type, num2str(enc_id), num2str(image_id), num2str(chan_id)));
end
...
for patient_id = params.patient_IDs
    ...
    parfor image_id = 1:9
        ...
        final_p_by_channels_image.EMS = nan(length(channel_ids_to_use), 9);
        % final_p_by_channels_image.EES = nan(length(channel_ids_to_use), 9);
        % anat_by_channels = strings(length(channel_ids_to_use), 1);
    
        % if exist(sprintf('%s\\%s\\%s\\%s\\enc%s_image%s\\t_test_p_values.mat',  params.output_folder, num2str(patient_id), comp_options{2}, params.btwn_trial_type, num2str(enc_id), num2str(image_id)), 'file')
            % load(sprintf("p_values_by_channel_%s_%s.mat", num2str(patient_id), num2str(image_id)))
        % else
            for chan_idx = 1:length(channel_ids_to_use)
                ...
                BI = struct();
                % params.btwn_trial_type = 'EES';
                % [BI.EES, WI.EES] = load_BT_data(patient_id, comp_options, enc_id, image_id, chan_id, params, 'EES');
                ...
                fprintf(" computing p values for p%s image%s chan%s\n", num2str(patient_id), num2str(image_id), num2str(chan_id));
                % final_p_by_channels_image.EES(chan_idx, image_id) = do_t_testing(EES_WI, EES_BI);
                final_p_by_channels_image.EMS(chan_idx, image_id) = do_t_testing(EMS_WI, EMS_BI);
            end
        % end
        ...
        % save_p_values(patient_id, comp_options, enc_id, image_id, params, final_p_by_channels_images, anat_by_channels, channel_ids_to_use)
    end
...
function integersList = extractIntegersFromFilenames(patient_id, comp_options, enc_id, image_id, params)
    file_to_load = sprintf('%s\\%s\\%s\\%s\\enc%s_image%s\\', ...
        params.output_folder, num2str(patient_id), comp_options{1}, params.btwn_trial_type, num2str(enc_id), num2str(image_id));
