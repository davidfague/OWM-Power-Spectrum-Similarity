...
for patient_id = params.patient_IDs
    patient_preprocessed_data_path = fullfile(params.preprocessed_data_location, sprintf('/CS%s/', num2str(patient_id)));
    load(fullfile(patient_preprocessed_data_path, "OWM_trialinfo.mat"), 'C');
    load(fullfile(patient_preprocessed_data_path, "D_OWM_t_bipolar.mat"), 'labelsanatbkedit');
    channel_ids_to_use = channel_ids_to_use(load_channels_from_file(patient_id, comp_options, enc_id, image_id, params));
    ...
    for image_id = 1:9
        ...
        if exist(sprintf('%s\\%s\\%s\\%s\\enc%s_image%s\\t_test_p_values.mat',  params.output_folder, num2str(patient_id), comp_options{2}, params.btwn_trial_type, num2str(enc_id), num2str(image_id)), 'file')
            ...
            for chan_idx = 1:length(channel_ids_to_use)
                ...
                EMS_WI  = EMS_WI(:,:,all(~isnan(EMS_WI),[1 2]));%remove nans trial pairs
                EMS_BI  = EMS_BI(:,:,all(~isnan(EMS_BI),[1 2]));%remove nans trial pairs
                sprintf("    computing p values for p%s image%s chan%s", num2str(patient_id), num2str(image_id), num2str(chan_id))
...
function channel_ids_to_keep = filter_channels_ids_to_use(channel_ids_to_use, original_labels)
    ...
    for chan_idx = 1:length(channel_ids_to_use)
        ...
        if all(~contains(label, excluded_terms))
            ...
            channel_ids_to_keep = [channel_ids_to_keep, chan_idx];
        end
    end
end
