...
params.btwn_trial_type = 'EES'; % default should be 'EMS'
target_enc_ids = 1; 
target_image_ids = [1];
...
% enc_win_IDs based on the encID where the thing is found.
    ...
    if params.within_item % the following could use params.output_folder instead of strrep(PS.source, PS_file, new)
        save_folder = fullfile(sprintf('%s/%d/corr BT WI/%s/enc%d_image%d', params.output_folder, patient_ID, params.btwn_trial_type, target_enc_ids, target_image_ids));
    else
        save_folder = fullfile(sprintf('%s/%d/corr BT BI/%s/enc%d_image%d', params.output_folder, patient_ID, params.btwn_trial_type, target_enc_ids, target_image_ids));
    end
