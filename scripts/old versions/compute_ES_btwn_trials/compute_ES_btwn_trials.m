...
%% constants
hellbender = false;
within_item = true; % computes control as either within item or between items
...
%cd('D:\Power Spectrum Similarity\A_main\') % cd('D:\Power Spectrum Similarity')%cd('C:\Users\david\Downloads\Power Spectrum Similarity')
if hellbender
...
if hellbender
    ...
    patient_IDs = [201901, 201910, 201907]; %#ok<NBRAK2>
    % output_folder = 'D:\Power Spectrum Similarity\AA_Processed Data\allpatients gammamod allregions allitem allenc'; %#ok<UNRCH>
...
for idx = 1%:length(patient_IDs)
    ...
    for target_enc_ids = 1%:3
        for target_image_ids = [1,3]
            fprintf("computing patient%d, enc%d, image%d \n", patient_ID, target_enc_ids, target_image_ids)
            [all_channels_save_file] = compute_all_between_trial_similarities(PS_file, target_enc_ids, target_image_ids, within_item);
        end
    end
...
%% main computations
function [all_channels_save_file] = compute_all_between_trial_similarities(PS_file, target_enc_ids, target_image_ids, within_item)
    [~, enc1_win_IDs, enc2_win_IDs, enc3_win_IDs, maint_win_IDs, ~, ~] = get_window_IDs(); % won't change each function call
    ...
    clearvars enc1_win_IDs enc2_win_IDs enc3_win_IDs
    ...
    for chan_idx = 1:length(unique_channel_IDs)
        ...
        if within_item % prevent matching trial with itself for within-item.
        ...
        unique_results = nan(length(enc_win_IDs), length(maint_win_IDs), size(unique_pairs, 1)); % Initialize a temporary array to store computations for each unique pair
        ...
        parfor k = 1:size(unique_pairs, 1)
            j = unique_pairs(k, 1);        % test trial index
            randtrial = unique_pairs(k, 2); % control trial index

            if any(any(isnan(chan_test_PSVs(:, :, j)))) || any(any(isnan(chan_ctrl_PSVs(:, :, randtrial))))
                error("nan detected")
            end
            
            % Compute and store the result for the unique pair
            unique_results(:, :, k) = compute_BT_similarity_matrix(chan_test_PSVs(:, :, j), chan_ctrl_PSVs(:, :, randtrial), enc_win_IDs, maint_win_IDs);
        end
    ...
    save(all_channels_save_file, "target_image_ids", "target_enc_ids", "test_rows", "control_rows", "unique_channel_IDs", "-v7.3")
end
