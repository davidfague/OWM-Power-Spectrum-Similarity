...
% load(fullfile(strcat(params.preprocessed_data_location, "\\OWM_trialinfo_",num2str(patient_id),".mat")), 'C') % images
...
for chan_id = [10]%[10 11 12 22 61 7 8 9 22 23 24 25 26 48 49 50 63 64 65 66]
    ...
    load(fullfile(sprintf('%s\\%s\\PSVs\\%s\\between_trials_enc%s_image%s\\all_channels_data', ...
        params.output_folder, num2str(patient_id), comp_options{2}, num2str(enc_id), num2str(image_id))))

    % Load BT-BI data for this channel
    load(fullfile(sprintf('%s\\%s\\PSVs\\%s\\between_trials_enc%s_image%s\\BT_%s.mat', ...
    ...
    subplot(1, 3, 1)
    result = all3_ES_matrix(:, :, Mtime, enc_id); % result = all3_ES_matrix(rows_without_nan, :, Mtime, enc_id);
    result = result(test_rows, :, :, :);
    result = result(chan_test_rows, :, :, :);
    imagesc(squeeze(mean(result, 1, 'omitnan')), [0 0.5])
    title('Within-Trial Similarity Mean across trials')
...
for chan_id = [10 11 12 22 61 7 8 9 22 23 24 25 26 48 49 50 63 64 65 66]
    [BT_BI_data, BT_BI_data_std, BT_WI_data, test_rows] = load_BT_data(patient_id, comp_options, enc_id, image_id, chan_id, params);
    ...
    hold on
    subplot(2,2,1);
    imagesc(squeeze(mean(BT_WI_data(:,:,:,:), [3 4], 'omitnan')))
    clim([0 0.08])
    title('BT WI mean across trial pairs and samples')

    hold on
    subplot(2,2,2);
    imagesc(squeeze(mean(BT_WI_data(:,:,1,1), [4], 'omitnan')))
    clim([0 0.08])
    title('BT WI single trial pair and sample')

    subplot(2,2,3);
    imagesc(squeeze(mean(BT_BI_data(:,:,:,:),[3 4], 'omitnan')))
    clim([0 0.08])
    title('BT BI mean across trial pairs and samples')

    subplot(2,2,4);
    imagesc(squeeze(mean(BT_BI_data(:,:,1,:),[4], 'omitnan')))
    clim([0 0.08])
...
function [BT_BI_data, BT_BI_data_std, BT_WI_data, test_rows] = load_BT_data(patient_id, comp_options, enc_id, image_id, chan_id, params)
    load(sprintf('%s\\%s\\PSVs\\%s\\between_trials_enc%s_image%s\\all_channels_data', ...
        params.output_folder, num2str(patient_id), comp_options{2}, num2str(enc_id), num2str(image_id)))

    % Load BT-BI data for this channel
    load(sprintf('%s\\%s\\PSVs\\%s\\between_trials_enc%s_image%s\\BT_%s.mat', ...
        params.output_folder, num2str(patient_id), comp_options{1}, num2str(enc_id), num2str(image_id), num2str(chan_id)))
    BT_BI_data = control_EMS_matrices; % Rename for clarity
    BT_BI_data_std = control_EMS_matrices_std; % Rename for clarity
...
hold on
subplot(1,3,1);
imagesc(squeeze(mean(BT_WI_data(:,:,:,:), [3 4])))
clim([0 0.08])
subplot(1,3,2);
