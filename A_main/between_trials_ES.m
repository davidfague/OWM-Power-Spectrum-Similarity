...
for idx = 1:length(patient_IDs)
    ...
    for target_enc_ids = 1%:3
        for target_image_ids = 1%:9
               [all_channels_save_file] = compute_all_between_trial_similarities(PS_file, target_enc_ids, target_image_ids);
        end
    end

end
function [all_channels_save_file] = compute_all_between_trial_similarities(PS_file, target_enc_ids, target_image_ids)
    % target_image_ids = 1;
    % target_enc_ids = 1;
    ...
    % Select the appropriate encoding window IDs based on the target encoding IDs
    ...
    save_folder = strrep(PS_file.Properties.Source, '.mat', sprintf('between_trials_enc%d_image%d', target_enc_ids, target_image_ids));
    if ~exist(save_folder, 'dir')
        mkdir(save_folder);
    end

    %% Initialize storage structures for all channels
    % all_test_EMS_matrices_by_chan = cell(length(unique_channel_IDs), 1);
    all_control_EMS_matrices_by_chan = cell(length(unique_channel_IDs), 1);

    %% Loop through each unique channel
    for chan_idx = 1:10%length(unique_channel_IDs)
        ...
        num_trials = size(chan_test_PSVs, 3);
        ...
        % Extract the relevant test trial data to avoid broadcasting
        chan_test_data = cell(1, num_trials);
        for j = 1:num_trials
            chan_test_data{j} = chan_test_PSVs(:, :, j);
        end
        ...
        parfor samp = 1:numSamples
            ...
            for j = 1:num_trials
                ...
                temp_control_EMS_matrices(:, :, j) = compute_BT_similarity_matrix(chan_test_data{j}, chan_ctrl_PSVs(:, :, randtrial), enc_win_IDs, maint_win_IDs);
            end
...
function [label_table, all_windowed_mean_PS_vectors] = get_cleaned_data(data_file)
    label_table = data_file.label_table;
    rows_with_nan = any(isnan(label_table.EMS_means), 2);
    rows_without_nan = ~rows_with_nan;
    label_table = label_table(rows_without_nan, :);
    all_windowed_mean_PS_vectors = data_file.all_windowed_mean_PS_vectors(:, :, rows_without_nan);

    % Remove rows from label_table and all_windowed_mean_PS_vectors if there are any NaNs in that row of all_windowed_mean_PS_vectors.
    reshaped_matrix = reshape(all_windowed_mean_PS_vectors, [], size(all_windowed_mean_PS_vectors, 3)); % Reshape the matrix to combine the first two dimensions (891x40) into one
    ...
    all_windowed_mean_PS_vectors = all_windowed_mean_PS_vectors(:, :, ~nan_vector);
    label_table = label_table(~nan_vector, :);
end
